![Banner](.attachments/pyroscope-banner.png)
# Pyroscope
![icon](.attachments/pyroscope-icon.png) </br>

</br>
Grafana Pyroscope is an open-source continuous profiling backend that collects, stores, and queries profiling data from your applications. It reveals resource usage - CPU, memory, and more - down to the line of code, making it easy to pinpoint performance bottlenecks and reduce cost. Integrated with Grafana alongside Prometheus, Loki, and Tempo, it adds profiles as a fourth pillar of observability, letting you correlate them with metrics, logs, and traces.
</br>
</br>

# ![icon](.attachments/d.png)/![icon](.attachments/k.png) Deployments

</br>

| Deployment           | Location  | Tags                        | Status       | Url                                                              |
| -------------------- | --------- | --------------------------- | ------------ | ---------------------------------------------------------------- |
| pyroscope-dev-2-vm-1 | dev-2-vm  | ![icon](.attachments/d.png) | **inactive** | https://pyroscope.margusm.dev/<br>http://192.168.40.112:4040<br> |

# References
- ![icon](.attachments/github-icon.png)  [Github mirror](https://github.com/margusmuru/homelab-pyroscope)
- ![icon](.attachments/url-icon.png)  https://grafana.com/oss/pyroscope/
- ![icon](.attachments/url-icon.png)  [Docs](https://grafana.com/docs/pyroscope/latest/)
